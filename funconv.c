...
//  функция интерполяции одной клетки поверхности (3D)
// n,p - аргументы функционального преобразователя (обороты и разрежение)
...
// возвращает интерполированное значение функции * 10         
int func_i3d(int n,int p,int a1,int a2,int a3,int a4,int n_s,int p_s,int n_l,int p_l)
{
   int a23,a14;  
   a23 = ((a2*10)+(((long)(a3-a2)*10)*(n-n_s))/n_l);
   a14 = (a1*10)+(((long)(a4-a1)*10)*(n-n_s))/n_l;
   return (a14+((((long)(a23-a14))*(p-p_s))/p_l));
} 

//функция интерполяции одного отрезка (2D)
...
{
  return ((a1*10)+(((long)(a2-a1)*10)*(n-n_s))/n_l);
}


//Функциональное преобразование частоты вращения в угол опережения
// (карта УОЗ для холостого хода)
...
//Функциональное преобразование частоты вращения в угол опережения
// (карта УОЗ для пуска двигателя)
//f - указатель на структуру используемого семейства характеристик
//n - частота вращения коленвала (мин-1)
// Возвращает значение угла опережения в целом виде * 40
int str_func(ecudata* d)
{
  ...
  if (i>=15) 
     ...
     i1 = i+1;
 return func_i2d(n,d->fn_dat->f_str[i],d->fn_dat->f_str[i1],(i*40)+200,40)*2;
}


// Функциональное преобразование нагрузки и частоты в
// угол опережения зажигания (основная - рабочая карта)
// Возвращает значение угла опережения в целом виде * 40
