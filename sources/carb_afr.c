...
void carbafr_control(void)
{
 ...
 if (d.sens.rpm < d.param.inj_lambda_rpm_thrd) //100 min-1
 {
 ...
 { //RPM > 100 min-1
  ...
  if (lambda_is_activated(2) && (d.sens.temperat > d.param.inj_lambda_temp_thrd) && (d.sens.rpm < CAFR_HLD_RPM_THRD) && (get_discharge() > d.param.fe_on_threshold))
  {
   if (d.sens.carb)
   { //throttle is opened
    if (d.sens.rpm > CAFR_IDL_RPM_THRD)
    {
     //control FE,--> IE=100%
