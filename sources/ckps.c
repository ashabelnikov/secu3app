...
#if !defined(HALL_SYNC) && !defined(CKPS_2CHIGN) && !defined(CKPS_NPLUS1) && !defined(CAM_SYNC) && !defined(ODDFIRE_ALGO)
...
typedef struct
{
 ...
 int16_t  advance_angle;              //!< required adv.angle * ANGLE_MULTIPLIER (òðåáóåìûé ÓÎÇ * ANGLE_MULTIPLIER)
...
{
 uint8_t _t, i;
 //translate from degrees to teeth (ïåðåâîäèì èç ãðàäóñîâ â çóáüÿ)
...
static void process_ckps_cogs(void)
{
 ...
 if (CHECKBIT(flags2, F_PNDDWL1) && !ckps.rising_edge_spark)
 {
  ...
  if (1!=ckps.chan_number || !CHECKBIT(flags2, F_BLKDWL1) || angle_to_spark > ANGLE_MAGNITUDE(66.0)) //prevent wrong dwell time on 1 cylinder engine
  {
   CLEARBIT(flags2, F_BLKDWL1);
   int32_t delay =  (angle_to_spark * (((int32_t)ckps.period_curr * ckps.degrees_per_cog_r) >> 10)) >> 6; //convert angle to delay
   delay-= ckps.cr_acc_time;    //apply dwell time
...
ISR(TIMER0_COMPA_vect)
{
 ...
 {//the countdown is over
  ...
  //start timer to recover missing tooth
