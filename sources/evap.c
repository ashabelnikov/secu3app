...
void evap_control(void)
{
 ...
 if ((d.sens.temperat > fw_data.exdata.evap_clt) && (d.sens.tps > fw_data.exdata.evap_tps_lo) && (d.sens.tps < fw_data.exdata.evap_tps_hi) && d.ie_valve && !d.fc_revlim && lambda_is_activated()
#ifdef SPEED_SENSOR
   && evap.state
#endif
    )
