...
void calc_lookup_args(void)
{
 ...
 for(fcs.la_f = F_WRK_POINTS_F-2; fcs.la_f >= 0; fcs.la_f--)
  if (fcs.la_rpm >= ram_extabs.rpm_grid_points[fcs.la_f]) break;

 //lookup table works from rpm_grid_points[0] and upper
 if (fcs.la_f < 0)  {fcs.la_f = 0; fcs.la_rpm = ram_extabs.rpm_grid_points[0];}
 if (fcs.la_rpm > ram_extabs.rpm_grid_points[F_WRK_POINTS_F-1]) fcs.la_rpm = ram_extabs.rpm_grid_points[F_WRK_POINTS_F-1];
 ...
 { //use grid table
  ...
  if (fcs.la_load > ram_extabs.load_grid_points[F_WRK_POINTS_L-1])
   fcs.la_load = ram_extabs.load_grid_points[F_WRK_POINTS_L-1];

  for(fcs.la_l = F_WRK_POINTS_L-2; fcs.la_l >= 0; fcs.la_l--)
   if (fcs.la_load >= ram_extabs.load_grid_points[fcs.la_l]) break;
  fcs.la_lp1 = fcs.la_l + 1;

  //update air flow variable (find nearest point)
  if (fcs.la_load > (ram_extabs.load_grid_points[fcs.la_l] + (ram_extabs.load_grid_sizes[fcs.la_l] / 2)))
 ...
 for(fcs.ta_i = F_TMP_POINTS-2; fcs.ta_i >= 0; fcs.ta_i--)
  if (fcs.ta_clt >= ((int16_t)ram_extabs.clt_grid_points[fcs.ta_i])) break;

 //lookup table works from clt_grid_points[0] and upper
 if (fcs.ta_i < 0)  {fcs.ta_i = 0; fcs.ta_clt = (int16_t)ram_extabs.clt_grid_points[0];}
 ...
 for(fcs.ga_i = F_TMP_POINTS-2; fcs.ga_i >= 0; fcs.ga_i--)
  if (fcs.ga_grt >= ((int16_t)ram_extabs.clt_grid_points[fcs.ga_i])) break;

 //lookup table works from clt_grid_points[0] and upper
 if (fcs.ga_i < 0)  {fcs.ga_i = 0; fcs.ga_grt = (int16_t)ram_extabs.clt_grid_points[0];}
 if (fcs.ga_grt > ((int16_t)ram_extabs.clt_grid_points[F_TMP_POINTS-1])) fcs.ga_grt = (int16_t)ram_extabs.clt_grid_points[F_TMP_POINTS-1];
...
int16_t idling_function(void)
{
 return simple_interpolation(fcs.la_rpm, _GB(f_idl[fcs.la_f]), _GB(f_idl[fcs.la_fp1]),
        ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 16);
}
...
int16_t crkclt_function(void)
{
 ...
 return simple_interpolation(fcs.ta_clt, (int8_t)ram_extabs.cts_crkcorr[fcs.ta_i], (int8_t)ram_extabs.cts_crkcorr[fcs.ta_i1],
        (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16);
}
...
uint8_t knock_attenuator_function()
{
 ...
 return simple_interpolation(rpm, ram_extabs.attenuator_table[i],
        ram_extabs.attenuator_table[i1], (i * 60) + 200, 60, 16) >> 4;
}
...
#endif
  return simple_interpolation(voltage, ram_extabs.coil_on_time[i], ram_extabs.coil_on_time[i1],
...
int16_t thermistor_lookup(uint16_t adcvalue, int16_t *lutab)
{
 ...
 uint16_t v_start = lutab[THERMISTOR_LOOKUP_TABLE_SIZE];
 //Voltage value at the end of axis in ADC discretes
 uint16_t v_end = lutab[THERMISTOR_LOOKUP_TABLE_SIZE+1];
 ...
 return (simple_interpolation(adcvalue, (int16_t)lutab[i], (int16_t)lutab[i1], //<--values in table are signed
...
int16_t airtemp_function(void)
{
 ...
 return simple_interpolation(t, (int8_t)ram_extabs.ats_corr[i], (int8_t)ram_extabs.ats_corr[i1], //<--values in table are signed
...
uint16_t inj_cranking_pw(void)
{
 ...
 return simple_interpolation(fcs.ta_clt, _GWU(inj_cranking[fcs.ta_i]), _GWU(inj_cranking[fcs.ta_i1]),  //<--values in table are unsigned
        (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 1) /*>> 0*/;
}
...
void calc_ve_afr(void)
{
 ...
 {//look into VE table
  fcs.vecurr = bilinear_interpolation(fcs.la_rpm, fcs.la_load,
        ...
        use_grid ? ram_extabs.load_grid_points[fcs.la_l] : (fcs.la_grad * fcs.la_l),
        ram_extabs.rpm_grid_sizes[fcs.la_f],
        use_grid ? ram_extabs.load_grid_sizes[fcs.la_l] : fcs.la_grad, 8) >> 3;
...
int16_t inj_timing_lookup(void)
{
 ...
 int16_t it = bilinear_interpolation(fcs.la_rpm, fcs.la_load,
        ...
        use_grid ? ram_extabs.load_grid_points[fcs.la_l] : (fcs.la_grad * fcs.la_l),
        ram_extabs.rpm_grid_sizes[fcs.la_f],
        use_grid ? ram_extabs.load_grid_sizes[fcs.la_l] : fcs.la_grad, 8);
...
uint8_t inj_iac_pos_lookup(prev_temp_t* p_pt, uint8_t mode)
{
 ...
 return simple_interpolation(t, mode ? _GBU(inj_iac_run_pos[i]) : _GBU(inj_iac_crank_pos[i]), mode ? _GBU(inj_iac_run_pos[i1]) : _GBU(inj_iac_crank_pos[i1]),  //<--values in table are unsigned
  (int16_t)ram_extabs.clt_grid_points[i], (int16_t)ram_extabs.clt_grid_sizes[i], 16) >> 4;
}
#endif
...
uint8_t inj_aftstr_en(void)
{
 ...
 return simple_interpolation(fcs.ta_clt, _GBU(inj_aftstr[fcs.ta_i]), _GBU(inj_aftstr[fcs.ta_i1]),  //<--values in table are unsigned
        (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4;
}
...
uint16_t inj_idlreg_rigidity(uint16_t targ_map, uint16_t targ_rpm)
{
 ...
 int16_t drpm = (((int32_t)abs(((int16_t)d.sens.inst_frq) - targ_rpm) * (int16_t)32 * k_rpm) / (ram_extabs.rpm_grid_points[RPM_GRID_SIZE-1] - ram_extabs.rpm_grid_points[0])) >> 0;
...
uint16_t inj_idling_rpm(void)
{
 ...
 return (simple_interpolation(fcs.ta_clt, _GBU(inj_target_rpm[fcs.ta_i]), _GBU(inj_target_rpm[fcs.ta_i1]),  //<--values in table are unsigned
         (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4) * 10;
}
#endif
...
uint16_t tpszon_function(void)
{
 return simple_interpolation(fcs.la_rpm, _GBU(inj_tpszon[fcs.la_f]), _GBU(inj_tpszon[fcs.la_fp1]),
        ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 32); //result is x 64
}
...
int16_t pa4_function(uint16_t adcvalue)
{
 ...
 return  simple_interpolation(adcvalue, (int8_t)ram_extabs.pa4_igntim_corr[i], (int8_t)ram_extabs.pa4_igntim_corr[i1], //<--values in table are signed
...
int16_t gdp_function(void)
{
 ...
 return bilinear_interpolation(fcs.la_rpm, tps,  //note that tps is additionally multiplied by 16
        ...
        ram_extabs.gasdose_pos[t][fcs.la_fp1],
        ram_extabs.rpm_grid_points[fcs.la_f],
        (TPS_AXIS_STEP*t),
        ram_extabs.rpm_grid_sizes[fcs.la_f],
        TPS_AXIS_STEP, 16) >> 4;
...
int16_t barocorr_lookup(void)
{
 ...
 uint16_t p_start = ram_extabs.barocorr[BAROCORR_SIZE];
 //Pressure value at the end of axis
 uint16_t p_end = ram_extabs.barocorr[BAROCORR_SIZE+1];
 ...
 return (simple_interpolation(press, (int16_t)ram_extabs.barocorr[i], (int16_t)ram_extabs.barocorr[i1], //<--values in table are signed
...
uint16_t cranking_thrd_rpm(void)
{
 ...
 return (simple_interpolation(fcs.ta_clt, ram_extabs.cranking_thrd[fcs.ta_i], ram_extabs.cranking_thrd[fcs.ta_i1],  //<--values in table are unsigned
         (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4) * 10;
}
...
uint16_t cranking_thrd_tmr(void)
{
 ...
 return (simple_interpolation(fcs.ta_clt, ram_extabs.cranking_time[fcs.ta_i], ram_extabs.cranking_time[fcs.ta_i1],  //<--values in table are unsigned
         (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4) * 10;
}
...
uint16_t smapaban_thrd_rpm(void)
{
 ...
 return (simple_interpolation(fcs.ta_clt, ram_extabs.smapaban_thrd[fcs.ta_i], ram_extabs.smapaban_thrd[fcs.ta_i1],  //<--values in table are unsigned
         (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4) * 10;
}
...
uint8_t knock_zone_val(void)
{
 ...
 for(f = F_WRK_POINTS_F-1; f >= 0; f--)
  if (rpm >= ram_extabs.rpm_grid_points[f]) break;
 //lookup table works from rpm_grid_points[0] and upper
 if (f < 0) f = 0;
 ...
 return ((uint8_t)(ram_extabs.knock_zones[t] >> f)) & 1;
}
...
uint16_t pwm_function(uint8_t mode)
{
 ...
 return bilinear_interpolation(fcs.la_rpm, fcs.la_load,
        ...
        use_grid ? ram_extabs.load_grid_points[fcs.la_l] : (fcs.la_grad * fcs.la_l),
        ram_extabs.rpm_grid_sizes[fcs.la_f],
        use_grid ? ram_extabs.load_grid_sizes[fcs.la_l] : fcs.la_grad, 64) >> 6;
 else
 return bilinear_interpolation(fcs.la_rpm, fcs.la_load,
...
{
 return simple_interpolation(fcs.ga_grt, ram_extabs.grheat_duty[fcs.ga_i], ram_extabs.grheat_duty[fcs.ga_i1],
        (int16_t)ram_extabs.clt_grid_points[fcs.ga_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ga_i], 16) >> 4;
}

uint16_t grv_delay(void)
{
 return simple_interpolation(fcs.ga_grt, ram_extabs.grv_delay[fcs.ga_i], ram_extabs.grv_delay[fcs.ga_i1],
        (int16_t)ram_extabs.clt_grid_points[fcs.ga_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ga_i], 2) >> 1;
}
#endif
...
uint16_t pwmiac_ucoef(void)
{
 ...
 return simple_interpolation(voltage, ram_extabs.pwmiac_ucoef[i], ram_extabs.pwmiac_ucoef[i1],
...
uint16_t aftstr_strokes(uint8_t mode)
{
 ...
 { //gas
  if (0==d.param.inj_aftstr_strokes1)
   return simple_interpolation(fcs.ta_clt, ram_extabs.inj_aftstr_strk1[fcs.ta_i], ram_extabs.inj_aftstr_strk1[fcs.ta_i1],  //<--values in table are unsigned
         (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4;
  else
 ...
 { //petrol
  if (0==d.param.inj_aftstr_strokes)
   return simple_interpolation(fcs.ta_clt, ram_extabs.inj_aftstr_strk0[fcs.ta_i], ram_extabs.inj_aftstr_strk0[fcs.ta_i1],  //<--values in table are unsigned
         (int16_t)ram_extabs.clt_grid_points[fcs.ta_i], (int16_t)ram_extabs.clt_grid_sizes[fcs.ta_i], 16) >> 4;
  else
...
#endif
 //Voltage value at the start of axis in ADC discretes
 uint16_t v_start = lutab[FTLS_LOOKUP_TABLE_SIZE];
 //Voltage value at the end of axis in ADC discretes
 uint16_t v_end = lutab[FTLS_LOOKUP_TABLE_SIZE+1];
...
int16_t injpwcoef_function(uint16_t adcvalue)
{
 ...
 return  simple_interpolation(adcvalue, ram_extabs.injpw_coef[i], ram_extabs.injpw_coef[i1],
        (i * v_step) + v_start, v_step, 4) >> 2;
}
#endif //SECU-3i
...
/*
 ...
 return (simple_interpolation_u(adcvalue, ram_extabs.maf_curve[i], ram_extabs.maf_curve[i1],
...
int16_t calc_ltft(uint8_t idx)
{
 ...
 return (bilinear_interpolation(fcs.la_rpm, fcs.la_load,
        ...
        use_grid ? ram_extabs.load_grid_points[fcs.la_l] : (fcs.la_grad * fcs.la_l),
        ram_extabs.rpm_grid_sizes[fcs.la_f],
        use_grid ? ram_extabs.load_grid_sizes[fcs.la_l] : fcs.la_grad, 128) >> 7) + 512;
}

uint8_t ltft_check_rpm_hit(void)
{
 uint16_t rpm = fcs.la_rpm;
 uint16_t band = (((uint32_t)ram_extabs.rpm_grid_sizes[fcs.la_f]) * PGM_GET_BYTE(&fw_data.exdata.ltft_cell_band)) >> 8;

 if (rpm <= (ram_extabs.rpm_grid_points[fcs.la_f] + band))
  return fcs.la_f; //near to lower point
 else if (rpm >= (ram_extabs.rpm_grid_points[fcs.la_fp1] - band))
...
uint8_t ltft_check_load_hit(void)
{
 ...
 { //use grid map
  uint16_t band = (((uint32_t)(ram_extabs.load_grid_sizes[fcs.la_l])) * PGM_GET_BYTE(&fw_data.exdata.ltft_cell_band)) >> 8;

  if (load <= (ram_extabs.load_grid_points[fcs.la_l] + band))
   return fcs.la_l; //near to lower point
  else if (load >= (ram_extabs.load_grid_points[fcs.la_lp1] - band))
   return fcs.la_lp1; //near to upper point
 }
...
uint16_t ftlscor_ucoef(void)
{
 ...
 return simple_interpolation(voltage, ram_extabs.ftlscor_ucoef[i], ram_extabs.ftlscor_ucoef[i1],
...
uint8_t lambda_zone_val(void)
{
 ...
 return ((uint8_t)(ram_extabs.lambda_zones[fcs.la_l] >> f)) & 1;
}
#endif
...
uint16_t fueldens_corr(void)
{
 ...
 return simple_interpolation_u(t, ram_extabs.fueldens_corr[i], ram_extabs.fueldens_corr[i1],
...
void calc_xtau(int32_t* pw1, int32_t* pw2)
{
 ...
 { //Acceleration
  xf = simple_interpolation(fcs.la_rpm, ram_extabs.xtau_xfacc[fcs.la_f], ram_extabs.xtau_xfacc[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 16) >> 4;

  tf = simple_interpolation_u(fcs.la_rpm, ram_extabs.xtau_tfacc[fcs.la_f], ram_extabs.xtau_tfacc[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 1);
 }
 else if (d.sens.mapdot < f_thrd)
 { //Decelaration
  xf = simple_interpolation(fcs.la_rpm, ram_extabs.xtau_xfdec[fcs.la_f], ram_extabs.xtau_xfdec[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 16) >> 4;

  tf = simple_interpolation_u(fcs.la_rpm, ram_extabs.xtau_tfdec[fcs.la_f], ram_extabs.xtau_tfdec[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 1);
 }
 else
 { //in between
  ...
  uint16_t xfa = simple_interpolation(fcs.la_rpm, ram_extabs.xtau_xfacc[fcs.la_f], ram_extabs.xtau_xfacc[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 16) >> 4;
  uint16_t xfd = simple_interpolation(fcs.la_rpm, ram_extabs.xtau_xfdec[fcs.la_f], ram_extabs.xtau_xfdec[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 16) >> 4;
  xf = simple_interpolation(d.sens.mapdot, xfd, xfa, f_thrd, dthrd, 1) >> 0;

  uint16_t tfa = simple_interpolation_u(fcs.la_rpm, ram_extabs.xtau_tfacc[fcs.la_f], ram_extabs.xtau_tfacc[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 1);

  uint16_t tfd = simple_interpolation_u(fcs.la_rpm, ram_extabs.xtau_tfdec[fcs.la_f], ram_extabs.xtau_tfdec[fcs.la_fp1],
                 ram_extabs.rpm_grid_points[fcs.la_f], ram_extabs.rpm_grid_sizes[fcs.la_f], 1);
