...
#define LTFT_MIN -126 //!< -126 / 512 = -0.246 (-24.6%)
#define LTFT_MAX  126 //!<  126 / 512 =  0.246 ( 24.6%)

/***/
#define LAMBDA_SWT_NUM 4

uint8_t ltft_state = 0;
uint16_t stat_tmr = 0;
uint8_t ltft_idx_r = 0;
uint8_t ltft_idx_l = 0;
int16_t ltft_corr = 0;
uint8_t idx_l = 0;
uint8_t idx_r = 0;
int16_t ltft_curr = 0;
 ...
 switch(ltft_state)
 {
  ...
  case 2: //perform correction of neighbour cells
  {
   if (ltft_idx_r != idx_r || ltft_idx_l != idx_l) //skip already corrected (current) cell
   {
    ...
    int8_t dist = (dist_l > dist_r) ? dist_l : dist_r;
    int16_t new_val = ltft_curr + (((((int32_t)ltft_corr) * PGM_GET_BYTE(&fw_data.exdata.ltft_learn_grad)) >> 8) / dist);
    restrict_value_to(&new_val, LTFT_MIN, LTFT_MAX);
