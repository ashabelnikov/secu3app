 ...
 * (–еализаци€ обработки поступающих данных дл€ приема/передачи через последовательный интерфейс (UART)).
 */
...
void process_uart_interface(struct ecudata_t* d)
{
 ...
 if (uart_is_packet_received())//прин€ли новый фрейм ?
 {
  ...
  switch(descriptor)
  {
   ...
   case OP_COMP_NC:
    if (_AB(d->op_actn_code, 0) == OPCODE_EEPROM_PARAM_SAVE) //прин€ли команду сохранени€ параметров
    {
     sop_set_operation(SOP_SAVE_PARAMETERS);
     _AB(d->op_actn_code, 0) = 0; //обработали
    }
    if (_AB(d->op_actn_code, 0) == OPCODE_CE_SAVE_ERRORS) //прин€ли команду чтени€ сохраненных кодов ошибок
    {
     sop_set_operation(SOP_READ_CE_ERRORS);
     _AB(d->op_actn_code, 0) = 0; //обработали
    }
    if (_AB(d->op_actn_code, 0) == OPCODE_READ_FW_SIG_INFO) //прин€ли команду чтени€ и передачи информации о прошивке
    {
     sop_set_operation(SOP_SEND_FW_SIG_INFO);
     _AB(d->op_actn_code, 0) = 0; //обработали
    }
#ifdef REALTIME_TABLES
    if (_AB(d->op_actn_code, 0) == OPCODE_LOAD_TABLSET) //прин€ли команду выбора нового набора таблиц
    {
     sop_set_operation(SOP_LOAD_TABLSET);
     _AB(d->op_actn_code, 0) = 0; //обработали
    }
    if (_AB(d->op_actn_code, 0) == OPCODE_SAVE_TABLSET) //прин€ли команду сохранени€ сохранени€ набора таблиц дл€ указанного типа топлива
    {
...
#ifdef SECU3T
    cams_vr_set_edge_type(d->param.ref_s_edge_type); //REF_S (ƒЌќ)
#endif
