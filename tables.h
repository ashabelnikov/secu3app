 /****************************************************************
 *       SECU-3  - An open source, free engine control unit
 *    Designed by Alexey A. Shabelnikov. Ukraine, Gorlovka 2007.
 *       Microprocessors systems - design & programming.
 *    contacts:
 *              http://secu-3.narod.ru
 *              ICQ: 405-791-931
 ****************************************************************/

/* —труктура распределени€ пам€ти программ блока управлени€ SECU-3
 *        ________________________
 *       |                        |
 *       |       код              |   
 *       |                        |
 *       |------------------------|
 *       | свободное пространство | 
 *       |------------------------| <--- новые данные и структуры добавл€ть с этого адреса
 *       |  резервные  параметры  |
 *       |------------------------|
 *       |   массив таблиц        |
 *       |                        |
 *       |------------------------|
 *       |     CRC16              |  - контрольна€ сумма прошивки без учета байт этой  
 *       |________________________|    контрольной суммы и байтов бутлоадера
 *       |                        |
 *       |  boot loader           |
 *        ------------------------
 */



#ifndef _TABLES_H_
#define _TABLES_H_

#include "bootldr.h"   //дл€ того чтобы знать значение SECONDBOOTSTART, и только

//определ€ем количество узлов интерпол€ции дл€ каждой функции
#define F_WRK_POINTS_F         16  
#define F_WRK_POINTS_L         16  
#define F_TMP_POINTS           16
/*#define F_TMP_POINTS_L       8*/
#define F_STR_POINTS           16                            
#define F_IDL_POINTS           16     

#define F_NAME_SIZE            16


//ќписывает одно семейство характеристик, дискрета ”ќ« = 0.5 град, дл€ температуры = 0.25 град. 
typedef struct 
{
  signed   char f_str[F_STR_POINTS];                       // функци€ ”ќ« на старте
  signed   char f_idl[F_IDL_POINTS];                       // функци€ ”ќ« дл€ ’’
  signed   char f_wrk[F_WRK_POINTS_L][F_WRK_POINTS_F];     // основна€ функци€ ”ќ« (3D)
  signed   char f_tmp[F_TMP_POINTS];                       // функци€ коррект. ”ќ« по температуре
  unsigned char name[F_NAME_SIZE];                         // ассоциированное им€ (им€ семейства)
}F_data;


//описывает дополнительные данные хранимые в прошивке
typedef struct
{
  char fw_signature_info[48];
}FirmwareData;

//описывает параметры системы
typedef struct
{
  unsigned char tmp_use;                        //признак комплектации ƒ“ќ∆-ом
  unsigned char carb_invers;                    //инверси€ концевика на карбюраторе
  unsigned char idl_regul;                      //поддерживать заданные обороты ’’ регулмрованием ”ќ«
  unsigned char fn_benzin;                      //номер набора характеристик используемый дл€ бензина
  unsigned char fn_gas;                         //номер набора характеристик используемый дл€ газа
  unsigned int  map_grad;                       //наклон шкалы расхода воздуха (кѕа)
  unsigned int  ephh_lot;                       //нижний порог Ёѕ’’ (мин-1)
  unsigned int  ephh_hit;                       //верхний порог Ёѕ’’ (мин-1)
  unsigned int  starter_off;                    //порог выключени€ стартера (мин-1)
  signed   int  press_swing;                    //перепад давлени€ при полностью открытом дросселе   (кѕа)
  unsigned int  smap_abandon;                   //обороты перехода с пусковой карты на рабочую  (мин-1) 
  signed   int  max_angle;                      //ограничение максимального ”ќ«
  signed   int  min_angle;                      //ограничение минимального ”ќ«
  signed   int  angle_corr;                     //октан-коррекци€ ”ќ«    
  unsigned int  idl_turns;                      //заданные обороты ’’ дл€ поддержани€ регулмрованием ”ќ«   
  signed   int  ifac1;                          //коэффициенты ѕ-регул€тора оборотов ’’, дл€ положительной и
  signed   int  ifac2;                          //отрицательной ошибке соответственно, 1...100 
  signed   int  MINEFR;                         //зона нечувствительности регул€тора (обороты)
  signed   int  vent_on;                        //температура включени€ вентил€тора
  signed   int  vent_off;                       //температура выключени€ вентил€тора  

  signed int  map_adc_factor;
  signed long map_adc_correction;
  signed int  ubat_adc_factor;
  signed long ubat_adc_correction;
  signed int  temp_adc_factor;
  signed long temp_adc_correction;
  
  unsigned short crc;                           //контрольна€ сумма данных этой структуры (дл€ проверки корректности данных после считывани€ из EEPROM)  
}params;


//================================================================================
//определ€ем адреса таблиц в прошивке отталкива€сь от бутлоадера

//размер переменной контрольной суммы параметров в байтах
#define PAR_CRC_SIZE   sizeof(unsigned short) 

//размер переменной контрольной суммы прошивки в байтах
#define CODE_CRC_SIZE   sizeof(unsigned short) 

//размер кода программы без учета контрольной суммы
#define CODE_SIZE (SECONDBOOTSTART-CODE_CRC_SIZE)

//количество наборов характеристик хранимых в пам€ти программ
#define TABLES_NUMBER          8   

//адрес контрольной суммы в прошивке
#define CODE_CRC_ADDR (SECONDBOOTSTART-CODE_CRC_SIZE)

//адрес массива таблиц - семейств характеристик
#define TABLES_START (CODE_CRC_ADDR-(sizeof(F_data)*TABLES_NUMBER))

//адрес структуры дефаултных параметров (параметров EEPROM по умолчанию)
#define DEFPARAM_START (TABLES_START-sizeof(params))

//адрес дополнительных параметров
#define FIRMWARE_DATA_START (DEFPARAM_START-sizeof(FirmwareData))

//================================================================================



//данные в таблицах по умолчанию
#pragma object_attribute=__root
const FirmwareData __flash fwdata@FIRMWARE_DATA_START=
{
  "SECU-3 firmware. Build ["__DATE__"]"
};

//данные в таблицах по умолчанию
#pragma object_attribute=__root
const F_data __flash tables[TABLES_NUMBER]@TABLES_START=
{
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x08,0x0C,0x10,0x14,0x14,0x14,  //пускова€ карта
0x0A,0x0A,0x0A,0x00,0x00,0x00,0x00,0x0A,0x19,0x28,0x37,0x37,0x37,0x37,0x37,0x37,  //’’ карта

0x04,0x05,0x06,0x07,0x0A,0x0C,0x10,0x15,0x1A,0x20,0x24,0x27,0x28,0x28,0x28,0x28,  //карта основного режима
, //карта температурной коррекции ”ќ«
'2','1','0','8','3',' ','С','в','†','≠','§','†','а','в',' ',' ',

,
'2','1','0','8','3',' ','Д','®','≠','†','ђ','®','з','≠','†','п',

,
'К','Ђ','†','б','б','®','™','†',' ','1','.','5',' ',' ',' ',' ',

,
'К','Ђ','†','б','б','®','™','†',' ','1','.','6',' ',' ',' ',' ',

,
'Н','®','Ґ','†',' ','1','.','7',' ',' ',' ',' ',' ',' ',' ',' ',

,
'Н','®','Ґ','†',' ','1','.','8',' ',' ',' ',' ',' ',' ',' ',' ',

,
'У','З','А','М','3','3','1',' ',' ',' ',' ',' ',' ',' ',' ',' ',

,
'У','З','А','М','3','3','1','7',' ',' ',' ',' ',' ',' ',' ',' '
};

//резервные параметры
#pragma object_attribute=__root
const params __flash def_param@DEFPARAM_START = 
{1,0,0,6,6,75,1250,1500,600,110,650,1920,-480,0,800,40,40,10,392,384,16384,8192,16384,8192,16384,8192,0};                                         

#pragma object_attribute=__root
const unsigned short __flash code_crc@CODE_CRC_ADDR = 0x0000;

#endif //_TABLES_H_
