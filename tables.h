...
//Параметры необходимые для функционирования подсистемы контроля детонации.
//Эти параметры хранятся в EEPROM отдельно от основных параметров. Для этих
//параметров нет резервной копии во FLASH (если необходимость возникнет, то 
//с реализацией проблем возникнуть не должно).
typedef struct
{
  unsigned char use_knock_channel;     //признак использования канала детенации
  unsigned char bpf_frequency;         //центральная частота полосового фильтра
  signed int k_wnd_begin_angle;        //начало детонационного окна (градусы)
  signed int k_wnd_end_angle;          //конец детонационного окна (градусы)
  //таблица усиления аттенюатора (зависимость от оборотов).
  unsigned char attenuator_table[KC_ATTENUATOR_LOOKUP_TABLE_SIZE];   
  unsigned char reserved[24];
  unsigned short crc;                  //контрольная сумма данных этой структуры
}kc_params;
...
{1,0,0,6,6,1920,1250,1500,600,6400,650,1600,-320,0,800,4,4,10,392,384,16384,8192,16384,8192,16384,8192,0,20,10,
 96,96,-320,320,240,410,392,1250,1500,0,0,0,0,0,0,/*crc*/0};                                         
